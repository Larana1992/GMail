...
public class WaitHelper {
	...
	public static void waitForElementIsClickable(WebElement webElement,
		...
		try {
			...
			logger.error("No such element with tag name: "
					+ webElement.getTagName());
		} catch (ElementNotVisibleException ex) {
			logger.error("Element not visible with tag name: "
					+ webElement.getTagName());
		}
	}
	...
	public static void waitForElements(List<WebElement> listWebElements,
			WebDriver driver) {
		WebDriverWait wait = new WebDriverWait(driver, MIDDLE_DELAY);
		try {
			wait.until(ExpectedConditions
					.visibilityOfAllElements(listWebElements));
		} catch (NoSuchElementException e) {
