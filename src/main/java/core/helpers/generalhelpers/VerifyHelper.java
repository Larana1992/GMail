...
public class VerifyHelper {
	...
	public static boolean verifyIsElementsPresent(List<WebElement> elements) {
		for (WebElement element : elements) {
			if (!verifyIsElementPresent(element))
				return false;
		}
		return true;
	}

	public static boolean verifyCorespondingColor(WebElement row,
			WebElement firstProperty, WebElement secondProperty) {
	...
	public static boolean verifyCorespondingColorOfList(
			List<WebElement> properties) {
		for (int i = 0; i < properties.size() - 3; i = i + 3) {
			if (!verifyCorespondingColor(properties.get(i),
					properties.get(i + 1), properties.get(i + 2)))
				return false;
		}
		return true;
	}
